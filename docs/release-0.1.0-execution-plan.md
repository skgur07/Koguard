...
- 상태: **실행 중 — 조사·어미 경계 탐지 보강**
- 기준일: 2026-09-01
- 기준 브랜치: `dev`
- 기능 기준 commit: `bb919046a455b09f75cb69c720b9753973dcf150`
...
3. 수정 대상은 현재 확인된 `공백/혼합 우회 + 한국어 조사·어미 경계`로 제한하고, 480건
...
현재 비-gold 진단은 `balanced` 63/240, `aggressive` 180/240, decoy 탐지 0/240이다. 이 수치는
480건의 독립 판정 전이므로 제품 정확도나 공개 성능으로 사용하지 않는다. 현재 확인된
`aggressive` 미탐 60건은 공백 또는 혼합 우회 뒤에 한국어 조사가 붙는 경계 사례다.

## 4. 실행 순서와 체크리스트

### R1. 조사·어미 경계 탐지 보강

`시  발은`, `ㅅ ㅂ이`처럼 공백·혼합 우회 뒤에 한국어 조사나 어미가 붙는 미탐을 먼저
수정한다. 문맥과 무관하게 등록 표현의 substring을 차단한다는 공개 정책이 이미 확정됐으므로
480건 전체 판정을 기다리지 않는다.

- [ ] `시  발은`, `ㅅ ㅂ이`와 혼합 separator 사례를 실패 테스트로 고정
- [ ] 공백·혼합 우회 뒤 한글 조사·어미가 붙는 경우 탐지
- [ ] 원문 span과 canonical term이 기존 계약을 유지하는지 검증
- [ ] 정상 decoy와 기존 hard-negative의 FP 증분 검증
- [ ] 최대 입력 길이와 representative benchmark 회귀 검증
- [ ] `strict`·`balanced`·`aggressive` 이동 규칙 검증
- [ ] 변경 근거와 알려진 한계를 문서화
...
| ID | 상태 | 내용 | 해제 조건 |
| --- | --- | --- | --- |
| B-01 | 열림 | 공백/혼합 우회 뒤 조사·어미 경계 미탐 후보 60건 | R1 정확도·회귀 gate 통과 |
